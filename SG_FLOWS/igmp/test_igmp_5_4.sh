...
# --------------------------------------------
# IGMPv3 change to exclude
# --------------------------------------------

print_info "\tSending IGMPv3 [change to exclude mode] with Sources=3 through IF2" | tee -a REPORTS/${func}/${test_function}.out
sudo python3 SW/igmp/igmp_generator.py -v 3 -s 3 -t change_to_exclude -r 0.1 -i $IF2 | tee -a REPORTS/${func}/${test_function}.out
...
print_info "\tSending IGMPv3 [change to exclude mode] with Sources=0 through IF2" | tee -a REPORTS/${func}/${test_function}.out
sudo python3 SW/igmp/igmp_generator.py -v 3 -s 0 -t change_to_exclude -r 0.1 -i $IF2 | tee -a REPORTS/${func}/${test_function}.out
