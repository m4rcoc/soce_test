...
# --------------------------------------------
# IGMPv3 block old sources
# --------------------------------------------

print_info "\tSending IGMPv3 [block old sources] with Sources=0 through IF2" | tee -a REPORTS/${func}/${test_function}.out
sudo python3 SW/igmp/igmp_generator.py -v 3 -s 0 -t block_old_sources -r 0.1 -i $IF2 | tee -a REPORTS/${func}/${test_function}.out
...
print_info "\tSending IGMPv3 [block old sources] with Sources=3 through IF2" | tee -a REPORTS/${func}/${test_function}.out
sudo python3 SW/igmp/igmp_generator.py -v 3 -s 3 -t block_old_sources -r 0.1 -i $IF2 | tee -a REPORTS/${func}/${test_function}.out
