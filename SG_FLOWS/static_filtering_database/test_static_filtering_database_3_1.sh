...
sshpass -p $password ssh -t -o StrictHostKeyChecking=no $username@$ip "echo $password | sudo -S timeout 100 tcpdump -i $MGT_PORT -w test_static_filtering_database_3_1.pcap" &
